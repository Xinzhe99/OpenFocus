...
> [!NOTE]
> 🎉 **2026.09.21 (3)**: New **Wipe compare** view — after rendering, press "Wipe" in the result panel to compare the fusion result against any source frame (or two outputs against each other) in one frame with a draggable divider, shared zoom and pan. Performance: **ECC registration now computes frame pairs in parallel** (~1.5x+ faster, frame count scales), the app **starts faster** (torch probe moved off the UI thread, UPX disabled in release builds), and idle memory drops when frames need no warping.
