    ...
    def __init__(self):
        ...
        self.translations = {
            'en': {
                ...
                'msg_recent_missing_title': 'File Not Found',
                'msg_recent_missing_text': 'This item no longer exists and has been removed from the recent list.',
                'action_save': 'Save',
                'action_save_as': 'Save as',
                'action_delete': 'Delete',
                'action_gpu_accel': 'GPU Acceleration',
                'msg_gpu_accel_hint': 'When unchecked, all processing is forced to CPU. Enable this if GPU drivers cause problems.',
                'action_restore_last_stack': 'Restore Last Stack on Startup',
                'action_align_cache': 'Registration Cache',
                'action_align_cache_hint': 'Store registration results next to the stack so re-opening it skips alignment',
                'btn_wipe': 'Wipe',
                'chk_quick_preview': 'Quick preview',
                'chk_quick_preview_hint': 'Render a downscaled draft first (fast). Uncheck for the full-resolution result. Preview drafts are not added to the output history.',
                'menu_theme': 'Theme',
                'theme_dark': 'Dark',
                'theme_light': 'Light',
                ...
                'update_available_text': 'OpenFocus {version} is available. Download the new version from the Releases page?',
                'btn_open_downloads': 'Open Downloads Page',
                'btn_later': 'Later',
                'update_up_to_date_title': 'Up to Date',
                'update_up_to_date_text': 'You are running the latest version of OpenFocus.',
                'update_check_failed_title': 'Update Check Failed',
                'update_check_failed_text': 'Could not reach GitHub. Please check your internet connection and try again later.',
                'msg_load_in_progress': 'Please wait: an image stack is still loading',
                'msg_loading_stack': 'Loading image stack…',
                'render_progress': 'Rendering',
                'btn_cancel_render': 'Cancel render',
                'msg_render_cancelled': 'Render cancelled',
                'msg_confirm_delete_title': 'Confirm Delete',
                'msg_confirm_delete_source_text': 'Delete {count} selected frame(s) from the source stack?\n\nThis cannot be undone.',
                ...
                'btn_compare_all': 'Compare All',
                'btn_compare_all_hint': 'Render the stack once per available fusion method so you can compare the results (output list + Wipe)',
                'btn_compare_stop': 'Stop',
                'msg_compare_progress': 'Comparing methods {i}/{n}: {method}',
                ...
                'wipe_hint': 'A/B wipe compare: drag the divider to compare, wheel to zoom, double-click to reset',
                'wipe_side_a': 'A:',
                'wipe_side_b': 'B:',
                'wipe_opt_source_current': 'Source: Current Frame',
                'wipe_opt_result': 'Fusion Result',
                'wipe_follow_source': 'Follow source',
                'wipe_follow_latest': 'Latest result',
                'wipe_result_fmt': 'Result {}',
                'wipe_frame_fmt': 'Frame {}',
                'wipe_need_result': 'Render a fusion or registration result first to use Wipe compare.',
                ...
                'info_preview_quality': 'Quality: Preview (quick draft — not saved to history)',
        ...
        consistency.</p>''',
            ...
            'zh': {
                ...
                'msg_recent_missing_title': '文件不存在',
                'msg_recent_missing_text': '该项已不存在，已从最近列表中移除。',
                'action_save': '保存',
                'action_save_as': '另存为',
                'action_delete': '删除',
                'action_gpu_accel': 'GPU 加速',
                'msg_gpu_accel_hint': '取消勾选后所有处理强制使用 CPU。当 GPU 驱动异常时可开启此选项。',
                'action_restore_last_stack': '启动时恢复上次图像栈',
                'action_align_cache': '配准缓存',
                'action_align_cache_hint': '将配准结果保存在图像栈旁，重新打开同一图像栈时跳过配准',
                'btn_wipe': '对比',
                'chk_quick_preview': '快速预览',
                'chk_quick_preview_hint': '先渲染缩小的草稿（速度快）。取消勾选渲染全分辨率结果。预览草稿不会进入输出历史。',
                'menu_theme': '主题',
                'theme_dark': '深色',
                'theme_light': '浅色',
                'menu_quick_start': '快速上手指南',
                'menu_check_updates': '检查更新',
                'menu_open_logs': '打开日志文件夹',
                'update_checking': '正在检查更新…',
                'update_available_title': '有可用更新',
                'update_available_text': 'OpenFocus {version} 已发布。是否前往发布页下载新版本？',
                'btn_open_downloads': '打开下载页',
                'btn_later': '稍后再说',
                'update_up_to_date_title': '已是最新版本',
                'update_up_to_date_text': '您正在使用最新版本的 OpenFocus。',
                'update_check_failed_title': '检查更新失败',
                'update_check_failed_text': '无法连接 GitHub。请检查网络连接后重试。',
                'msg_load_in_progress': '请稍候：图像栈仍在加载中',
                'msg_loading_stack': '正在加载图像栈…',
                'render_progress': '渲染中',
                'btn_cancel_render': '取消渲染',
                'msg_render_cancelled': '渲染已取消',
                'msg_confirm_delete_title': '确认删除',
                'msg_confirm_delete_source_text': '确定要从源图像栈中删除选中的 {count} 帧？\n\n此操作无法撤销。',
                ...
                'btn_compare_all': '对比全部',
                'btn_compare_all_hint': '依次用每个可用的融合算法渲染同一图像栈，结果进入输出列表供对比',
                'btn_compare_stop': '停止',
                'msg_compare_progress': '对比渲染 {i}/{n}：{method}',
                ...
                'wipe_hint': 'A/B 划分对比：拖动分割线比较两图，滚轮缩放，双击复位',
                'wipe_side_a': 'A:',
                'wipe_side_b': 'B:',
                'wipe_opt_source_current': '源帧: 当前帧',
                'wipe_opt_result': '融合结果',
                'wipe_follow_source': '跟随源图',
                'wipe_follow_latest': '最新结果',
                'wipe_result_fmt': '结果 {}',
                'wipe_frame_fmt': '第 {} 帧',
                'wipe_need_result': '请先渲染融合或配准结果再使用对比功能。',
                ...
                'info_preview_quality': '质量: 预览（快速草稿——不保存到历史）',
